...
SANITY_API_TOKEN = os.getenv("SANITY_API_TOKEN")  # optional if public dataset

if not SANITY_PROJECT_ID:
    st.warning("Zet SANITY_PROJECT_ID als omgevingsvariabele voordat je de app start.")

GROQ_BASE = f"https://{SANITY_PROJECT_ID}.api.sanity.io/v2024-01-01/data/query/{SANITY_DATASET}"

HEADERS = {}
if SANITY_API_TOKEN:
    HEADERS["Authorization"] = f"Bearer {SANITY_API_TOKEN}"

def run_groq(query: str) -> List[Dict[str, Any]]:
    """Voer een GROQ query uit en retourneer resultaten als lijst van dicts."""
    params = {"query": query}
    resp = requests.get(GROQ_BASE, params=params, headers=HEADERS, timeout=30)
...
# -------------------------
def dataset_to_dataframe(docs: List[Dict[str, Any]]) -> pd.DataFrame:
    """Converteer ruwe documenten naar een platte DataFrame (heuristisch)."""
    if not docs:
        return pd.DataFrame()
    df = pd.json_normalize(docs)
    return df

def sample_persona_from_df(df: pd.DataFrame, seed: int = None) -> Dict[str, Any]:
    """Genereer een eenvoudige synthetic persona op basis van dataset samenvattingen."""
    rng = np.random.default_rng(seed)
    persona = {}
    # Basis: kies een representatieve rij of combineer attributen
    if not df.empty:
        row = df.sample(n=1, random_state=seed).iloc[0].to_dict()
        # Heuristische mapping: probeer veelvoorkomende velden te gebruiken
        persona["name"] = row.get("name") or f"Persona_{rng.integers(1000)}"
        persona["age"] = row.get("age") or int(rng.integers(18, 80))
        persona["gender"] = row.get("gender") or row.get("sex") or "onbekend"
        persona["condition"] = row.get("diagnosis") or row.get("condition") or "onbekend"
        persona["quote"] = row.get("quote") or row.get("summary") or ""
        # tags: combineer trefwoorden als aanwezig
        tags = []
        for col in ["tags", "keywords", "issues", "challenges"]:
            val = row.get(col)
            if isinstance(val, list):
                tags.extend(val)
            elif isinstance(val, str):
                tags.extend([t.strip() for t in val.split(",") if t.strip()])
        persona["tags"] = list(dict.fromkeys(tags))[:6]
        persona["context"] = row.get("context") or row.get("work") or ""
        persona["goals"] = row.get("goals") or ""
        persona["frustrations"] = row.get("frustrations") or ""
    else:
...
# -------------------------
# Data justice / bias / hallucination checks
# -------------------------
def evaluate_bias(df: pd.DataFrame, sensitive_cols: List[str] = None) -> Dict[str, Any]:
    """Eenvoudige bias-evaluatie: controleer representatie van gevoelige groepen."""
    if df.empty:
        return {"score": None, "notes": "Geen data beschikbaar"}
    sensitive_cols = sensitive_cols or ["gender", "age_group", "ethnicity"]
    notes = []
    scores = []
    for col in sensitive_cols:
        if col in df.columns:
            counts = df[col].value_counts(normalize=True, dropna=True)
            # detecteer dominante groep (>70%)
            top_share = counts.iloc[0]
            if top_share > 0.7:
                notes.append(f"{col}: dominante groep ({counts.index[0]}) {top_share:.0%}")
                scores.append(0.3)
            else:
                notes.append(f"{col}: redelijk verdeeld (top {top_share:.0%})")
                scores.append(0.8)
        else:
            notes.append(f"{col}: kolom niet aanwezig")
            scores.append(0.5)
    # combineer tot 0..1 (1 = laag bias)
    score = float(np.mean(scores))
    return {"score": round(score, 2), "notes": notes}

def evaluate_hallucination(persona: Dict[str, Any], df: pd.DataFrame) -> Dict[str, Any]:
    """Eenvoudige hallucination-check: vergelijk persona attributen met dataset frequenties."""
    if df.empty:
        return {"score": None, "notes": ["Geen dataset om te vergelijken."]}
    notes = []
    score_components = []
    # Age plausibility: check of leeftijd binnen dataset range
    if "age" in persona:
        ages = pd.to_numeric(df.get("age", pd.Series(dtype=float)), errors="coerce").dropna()
        if not ages.empty:
            min_a, max_a = int(ages.min()), int(ages.max())
            if persona["age"] < min_a or persona["age"] > max_a:
                notes.append(f"Leeftijd {persona['age']} buiten dataset range ({min_a}-{max_a})")
                score_components.append(0.3)
            else:
                notes.append(f"Leeftijd {persona['age']} binnen dataset range")
                score_components.append(0.9)
        else:
            notes.append("Geen leeftijdsdata in dataset")
            score_components.append(0.6)
    # Condition plausibility: check if condition appears in dataset keywords
    cond = persona.get("condition", "").lower()
    if cond:
        # zoek in diagnosis/condition/keywords columns
        text_cols = []
        for c in ["diagnosis", "condition", "keywords", "tags", "summary"]:
            if c in df.columns:
                text_cols.append(df[c].astype(str).str.lower().fillna(""))
        if text_cols:
            combined = pd.concat(text_cols, axis=0).astype(str)
            contains = combined.str.contains(cond, na=False).any()
            if contains:
                notes.append(f"Condition '{persona['condition']}' komt voor in dataset")
                score_components.append(0.9)
            else:
                notes.append(f"Condition '{persona['condition']}' niet gevonden in dataset")
                score_components.append(0.4)
        else:
            notes.append("Geen tekstkolommen om condition te verifiëren")
            score_components.append(0.6)
    # Combineer
    score = float(np.mean(score_components)) if score_components else None
    return {"score": round(score, 2) if score is not None else None, "notes": notes}

def evaluate_data_justice(df: pd.DataFrame) -> Dict[str, Any]:
    """Eenvoudige data justice checks: consent, sensitive fields, provenance heuristics."""
    notes = []
    score_components = []
    # Check for consent/consent_flag column
    if "consent" in df.columns or "consent_given" in df.columns:
        consent_col = "consent" if "consent" in df.columns else "consent_given"
        consent_rate = df[consent_col].astype(bool).mean()
        notes.append(f"Consent rate: {consent_rate:.0%}")
        score_components.append(min(1.0, consent_rate + 0.1))
    else:
        notes.append("Geen expliciete consent-kolom gevonden")
        score_components.append(0.5)
    # Check for provenance / source column
    if any(c in df.columns for c in ["source", "provenance", "origin"]):
        notes.append("Provenance kolom aanwezig")
        score_components.append(0.9)
    else:
        notes.append("Geen provenance informatie")
        score_components.append(0.5)
    # Check for sensitive attributes being stored without masking
    sensitive_present = any(c in df.columns for c in ["ssn", "national_id", "address", "phone"])
    if sensitive_present:
        notes.append("Potentieel gevoelige velden aanwezig (controleer masking)")
        score_components.append(0.2)
    else:
        notes.append("Geen direct gevoelige velden gedetecteerd")
        score_components.append(0.9)
...
# Streamlit UI
# -------------------------
st.set_page_config(page_title="Synthetic User + Data Justice Assistent", layout="wide")
st.title("Synthetic User & Data Justice Assistent")

with st.sidebar:
    st.header("Dataset via GROQ")
    example_query = '*[_type == "participant"]{name, age, gender, diagnosis, tags, summary, consent, source}'
    st.markdown("**GROQ query** (pas aan indien nodig)")
    groq_query = st.text_area("GROQ", value=example_query, height=160)
    st.markdown("**Opsies**")
    seed = st.number_input("Random seed (persona)", min_value=0, max_value=99999, value=42)
    st.markdown("---")
    st.markdown("**Checks**")
    check_sensitive = st.multiselect("Sensitive columns (voor bias checks)", options=["gender","age_group","ethnicity","consent","source"], default=["gender","consent"])
    st.button("Haal dataset op", key="fetch")

# Main layout: twee kolommen
col1, col2 = st.columns([2, 1])

with col1:
    st.subheader("Dataset preview")
    try:
        docs = run_groq(groq_query)
    except Exception as e:
        st.error(f"Fout bij ophalen dataset: {e}")
        docs = []
    df = dataset_to_dataframe(docs)
    if df.empty:
        st.info("Geen data gevonden met de huidige GROQ query. Gebruik de voorbeeldquery of pas aan.")
    else:
        st.dataframe(df.head(50))

    st.subheader("Genereer synthetic persona")
    if st.button("Genereer persona"):
        persona = sample_persona_from_df(df, seed=seed)
        st.session_state["persona"] = persona
    persona = st.session_state.get("persona")
    if persona:
        st.markdown("### Persona kaart")
        st.markdown(f"**Naam:** {persona.get('name')}")
        st.markdown(f"**Leeftijd:** {persona.get('age')}")
        st.markdown(f"**Gender:** {persona.get('gender')}")
        st.markdown(f"**Condition:** {persona.get('condition')}")
        st.markdown(f"**Quote:** _{persona.get('quote')}_")
        st.markdown(f"**Tags:** {', '.join(persona.get('tags', []))}")
        st.markdown(f"**Context:** {persona.get('context')}")
        st.markdown(f"**Goals:** {persona.get('goals')}")
        st.markdown(f"**Frustrations:** {persona.get('frustrations')}")
        st.markdown("---")
        st.markdown("**Explainable AI**")
        st.info("Deze persona is gegenereerd op basis van patronen in de dataset. Bekijk de checks rechts voor betrouwbaarheid en risico's.")

with col2:
    st.subheader("Validation & Risk overview")
    # Bias
    bias_res = evaluate_bias(df, sensitive_cols=check_sensitive)
    halluc_res = evaluate_hallucination(persona or {}, df)
    dj_res = evaluate_data_justice(df)

    st.metric("Bias score (1 = laag bias)", value=bias_res["score"] if bias_res["score"] is not None else "N/A")
    st.write("**Bias notes**")
    for n in bias_res["notes"]:
        st.write("-", n)

    st.metric("Hallucination plausibility (1 = hoog betrouwbaar)", value=halluc_res["score"] if halluc_res["score"] is not None else "N/A")
    st.write("**Hallucination notes**")
    for n in halluc_res["notes"]:
        st.write("-", n)

    st.metric("Data Justice score (1 = goed)", value=dj_res["score"])
    st.write("**Data Justice notes**")
    for n in dj_res["notes"]:
        st.write("-", n)

    st.markdown("---")
    st.subheader("Acties en aanbevelingen")
    if dj_res["score"] < 0.6:
        st.warning("Data Justice score laag — voer een review uit op consent en provenance.")
    if bias_res["score"] is not None and bias_res["score"] < 0.6:
        st.warning("Bias detectie: oververtegenwoordiging gedetecteerd. Overweeg herweging of data-augmentatie.")
    if halluc_res["score"] is not None and halluc_res["score"] < 0.6:
        st.warning("Hallucinatie risico: persona attributen lijken niet goed te matchen met dataset.")

    st.markdown("**Verbeterstappen**")
    st.write("- Review dataset provenance en consent.")
    st.write("- Voeg meer diverse bronnen toe als groepen ondervertegenwoordigd zijn.")
    st.write("- Laat domeinexperts de persona en de explainability controleren.")
    st.button("Export rapport (JSON)", key="export")

# Footer: eenvoudige full-report JSON (download)
if st.button("Genereer Full Report JSON"):
    report = {
        "persona": st.session_state.get("persona"),
        "bias": bias_res,
        "hallucination": halluc_res,
        "data_justice": dj_res,
        "dataset_summary": {
            "n_rows": len(df),
    ...
    st.download_button("Download report.json", data=json.dumps(report, indent=2), file_name="data_justice_report.json", mime="application/json")
