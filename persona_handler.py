...
class PersonaSchema(BaseModel):
    ...
    initials:           str         =   Field(description="Initialen van de persona")
    ...
    geslacht:           str         =   Field(description="Geslacht van de persona")
    diagnose:           str         =   Field(description="Type reumatische aandoening")
    quote:              str         =   Field(description="Hededaagse uitdagingen in eigen woorden")
    tags:               list[str]   =   Field(description="Lijst van tags die de persona beschrijven")
    context:            str         =   Field(description="Korte beschrijving van de context waarin deze persona leeft")
    goals:              str         =   Field(description="Doelen of behoeften")
    frustrations:       str         =   Field(description="Frustraties of pijnpunten")
    tech_support:       str         =   Field(description="niveau van digitale vaardigheid van persona")
    data_bronnen:       list[str]   =   Field(description="Lijst van gebruikte datasets")
    sleutelfactoren:    list[str]   =   Field(description="Lijst van belangrijke factoren")
    xai_uitleg:         str         =   Field(description="uitleg van de belangrijkste factoren die de score beïnvloeden")
...
class PersonaHandler:
    ...
    def get_evaluation_dir_path(self) -> str:
        return "./evaluations/"
        ...
        for bestand in os.listdir(self.get_persona_dir_path()):
            if bestand.endswith(".json"):
                with open(self.get_persona_dir_path() + bestand, "r", encoding="utf-8") as f:
                    persona_file = json.load(f)
                    persona_file["id"] = len(personas) + 1
                    personas.append(persona_file)
        return personas
        ...
        return ChatPromptTemplate.from_messages([
            ("human", 
             """### Wie ben je
                ...
                ### Output (JSON)
                {{
                "naam": "naam van persona",
                "initials": "initialen van persona",
                "samenvatting": "neutrale beschrijving",
                "leeftijd": leeftijd in jaren,
                "geslacht": "geslacht van persona",
                
                "diagnose": "type reumatische aandoening",
                "quote": "Hededaagse uitdagingen in eigen woorden",
                
                "tags": ["tag1", "tag2"],
                "context": "korte beschrijving van de context waarin deze persona leeft",
                "goals": "doelen of behoeften",
                "frustrations": "frustraties of pijnpunten",

                "tech_support": "niveau van digitale vaardigheid van persona",
                "data_bronnen": ["gebruikte dataset 1", "gebruikte dataset 2"],

                "sleutelfactoren": ["factor 1", "factor 2"],
                "xai_uitleg": "uitleg van de belangrijkste factoren die de score beïnvloeden",
    ...
    def generate_persona(self):
        ...
        resultaat = self.write_json_file(json.dumps(persona.model_dump(), ensure_ascii=False))

        

        return persona

    def load_existing_persona_names(self):
        names = []
        ...
        for bestand in os.listdir(self.get_persona_dir_path()):
            if bestand.endswith(".json"):
                with open(self.get_persona_dir_path() + bestand, "r", encoding="utf-8") as f:
                    persona_file = json.load(f)
                    names.append(persona_file.get("naam"))
    ...
    def parse_persona(self, state: PersonaState) -> dict:
        ...
        chain = self.get_bias_justice_validator() | structured_llm
    ...
    def generate_single_persona(self):
    ...
    def generate_persona_evaluations(self, persona_json):
        ...
        # spreek altijd dezelfde persoon
        # dan hoeft alleen die persona gevalideerd te worden
    ...
    def write_csv_file(self, personas_output):
        # ============================================================
        # CSV EXPORT
        # ============================================================
        csv_pad = self.get_evaluation_dir_path() + "personas_evaluatie.csv"
    ...
    def write_json_file(self, personas_output):
        
        # TODO: WSS BREEKT DEZE CODE, CONTROLEER HET GEGENEREERDE JSON BESTAND


        # ============================================================
        # JSON EXPORT
        # ============================================================
        json_pad = self.get_evaluation_dir_path() + "personas_evaluatie.json"
